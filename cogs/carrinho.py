...
class Carrinho(commands.Cog):
    ...
    async def carrinho(self, user, thread, ):
        ...
        conn = sqlite3.connect('produtos.db')
        cursor = conn.cursor()
        cursor.execute("SELECT produto_id, usuario, quantia FROM carrinho WHERE usuario = ?", (idusuario,))
        carrinhos = cursor.fetchall()
        conn.close()
        ...
        async def resposta_cancelar(interact:discord.Interaction):
            ...
            conn = sqlite3.connect('produtos.db')
            cursor = conn.cursor()
            cursor.execute("DELETE FROM carrinho WHERE usuario = ?", (idusuario,))
            conn.commit()
            conn.close()
            ...
            async def select_resposta(interaction: discord.Interaction):
                ...
                class EditarProdutoModal(discord.ui.Modal):
                    ...
                    async def on_submit(self, interaction: discord.Interaction):
                        ...
                        if eh_inteiro(self.edicao.value):
                            if float(self.edicao.value) <= 0:
                                print('menor que 0')
                                conn = sqlite3.connect('produtos.db')
                                cursor = conn.cursor()
                                cursor.execute(f'''DELETE FROM carrinho WHERE produto_id = ? AND usuario = ?''', 
                                            (escolha, idusuario))
                                conn.commit()
                                conn.close()
                                ...
                                conn = sqlite3.connect('produtos.db')
                                cursor = conn.cursor()
                                cursor.execute(f'''UPDATE carrinho SET quantia = ? WHERE produto_id = ? AND usuario = ?''', 
                                            (self.edicao.value, escolha, idusuario))
                                conn.commit()
                                conn.close()
                                await interaction.response.send_message("Produto editado com sucesso!", ephemeral=True)
                                await cog1.carrinho(user, thread)
